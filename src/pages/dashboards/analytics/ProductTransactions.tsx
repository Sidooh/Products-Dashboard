...
import { useGetProductTransactionsQuery } from '@/services/analyticsApi';
...
const ProductTransactions = () => {
    ...
    const { data, isError, error, isLoading, isSuccess, refetch, isFetching } =
        useGetProductTransactionsQuery(bypassCache);
                    ...
                    .map((product, i) => (
                        <div key={`product-${i}`} className="flex items-center space-x-2">
                            <Checkbox
                                ...
                                onCheckedChange={(c) => handleCheckedTelcos(c, product)}
                            />
                            <label
                                htmlFor="terms"
